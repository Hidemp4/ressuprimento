...
export class RackFrame {
  constructor(
    ...
    const frameMaterial = new THREE.MeshToonMaterial({ color: 0xf1f1f1 }); // azul industrial
    ...
    if (showKnapp) {
      ...
      knappPlate.position.set(-13, height + 0.9, 0.1 + 3.6);
      this.group.add(knappPlate);
    }

    // === PLACA DO NÚMERO (label, ex: "K12") ===
    if (label !== null) {
      ...
      textPlane.position.set(-13, height + 0.6, thickness + 3.8);
